...
class InquiryCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $transactions = DB::table($this->transactions.' as trx')->select('trx.uuid', 'trx.partner_trx_no', 'trx.reference_no', 'trx.created_at')->where('trx.status', 'PENDING')->whereBetween('created_at', [$yesterday, $now])->get();

        if(!empty($transactions)){
            foreach ($transactions as $key => $transaction) {
                if(empty($transaction->partner_trx_no)){
                    $newDateTime = Carbon::parse($transaction->created_at)->addMinutes(5);
                    if($newDateTime <= $now){
                        DB::table($this->transactions)
                            ->where('uuid', $transaction->uuid)
                            ->update(['status' => 'FAIL', 'updated_at' => Carbon::now()]);
